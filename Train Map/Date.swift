...
extension Date {
    ...
    func bankHoliday() -> (Bool, Bool) {
        ...
        let components = calendar.dateComponents([.day, .month, .year, .weekday], from: self)
        ...
        if month == 1 {
            ...
            var days_to_add = 0
            let firstWeekday = calendar.component(.weekday, from: formatter.date(from: String(year!) + "-1-1")!)
            if (firstWeekday == 1) {
                days_to_add = 1
            }
            if (firstWeekday == 7) {
        ...
        if (month == 3) || (month == 4) {
            ...
            let easterDate = calendar.dateComponents([.day, .month], from:Date.easter(year!))
            let easterDay = easterDate.day
            let easterMonth = easterDate.month
        ...
        if (month == 5) {
            ...
            let firstWeekday = calendar.component(.weekday, from: formatter.date(from: String(year!) + "-5-1")!)
            let days_to_add = (2 - firstWeekday) % 7 // how long to get to the first monday?
            if day == (1 + days_to_add) {
                return (true, true)
            }
            
            //similar logic for the last Monday in May
            let lastWeekday = calendar.component(.weekday, from: formatter.date(from: String(year!) + "-5-31")!)
            let days_to_subtract = (lastWeekday - 2) % 7
        ...
        if (month == 8) {
            ...
            let firstWeekday = calendar.component(.weekday, from: formatter.date(from: String(year!) + "-8-1")!)
            let days_to_add = (2 - firstWeekday) % 7 // how long to get to the first monday?
            if day == (1 + days_to_add) {
            ...
            let lastWeekday = calendar.component(.weekday, from: formatter.date(from: String(year!) + "-8-31")!)
            let days_to_subtract = (lastWeekday - 2) % 7
            if (day == (31 - days_to_subtract)) {
                return (true, false) //England only
            }
        ...
        if (month == 11) {
            ...
            let andrewsWeekday = calendar.component(.weekday, from: formatter.date(from: String(year!) + "-11-30")!)
        ...
        if (month == 12) {
            ...
            if (day == 1) || (day == 2) {
                let andrewsWeekday = calendar.component(.weekday, from: formatter.date(from: String(year!) + "-12-" + String(day!))!)
                if (andrewsWeekday == 2) {
                    return (false, true) //Scotland only
                }
            }
            ...
            let christmasWeekday = calendar.component(.weekday, from: formatter.date(from: String(year!) + "-12-25")!)
            var days_to_add = 0
